...
| `--agent-attach HOST:PORT` | The same protocol and the same commands, aimed at the console **already running in a window** instead of a fresh one: this process becomes a thin client, forwarding stdin to that session and its answers back to stdout. Loopback only; the first line it sends is the shared secret, read from `PRISME_AGENT_SECRET` or from `--agent-secret VALUE`. Needs no `<rom>` — the cartridge is the one being played. The window opens that port only for the duration of one `Jouer le passage` request and closes it after, so this is not a way to attach to an arbitrary session. Frames a `step`/`press` asks for are emulated one per iteration of the event loop, so the player sees them play out; between two commands the console is held still. |
| `--agent-secret VALUE` | The secret for `--agent-attach`, when the environment cannot carry it (Windows shells have no `VAR=value command` form). |
